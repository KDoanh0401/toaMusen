...
def register_new_user(user_name):
    ...
    image_paths = [input(f"Enter the path for image {i+1}: ").strip() for i in range(13)]

    face_crop = detect_and_crop_face(image_paths)

    augmented_images = augment_images(face_crop, num_augmented=20)  
...
def recognize_face_realtime(threshold=0.85):
    global index, user_map
...
def main():
    ...
    if mode == '1':
        recognize_face_realtime(threshold=0.6)
    elif mode == '2':
